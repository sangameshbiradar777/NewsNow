...
function getCategoryNewsSection(newsObjects, heading) {
  ...
  categoryNewsSectionElement.setAttribute('class', 'category-section section-category-news');
  ...
  newsObjects.forEach(newsObject => {
    ...
    <a href="${newsObject.url}" target="_blank" class="more-news__item__link">
      <div class="more-news__item__text-container">
        <span class="featured-grid__item__author">${processDate(newsObject.created)}</span>
        <h2 class="more-news__item__title">${compressText(newsObject.title, 70)}</h2>
      </div>
